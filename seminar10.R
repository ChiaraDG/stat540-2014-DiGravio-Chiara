...
for (i in 1:nfold) {
  ...
  traindat.fold <- traindat.fold[c(as.numeric(rownames(top.fold))), ]
  testdat.fold <- testdat.fold[c(as.numeric(rownames(top.fold))), ]
...
yhat.knn <- knn(train = t(trainDat), test = t(testDat), cl = train.es$LnStatus, k = 10,prob=TRUE)
# Store the prediction error for each kk within this fold
...
# Add a method: Artificial Neural Network
ngenes <- 50
nmethod <- 8  #number of methods you plan to compare. 
# add number of run
nrun <- 20
...
for(j in 1:nrun){
pr.err <- matrix(-1, nfold, nmethod, dimnames = list(paste0("Fold", 1:nfold), 
                                                     c("1NN", "5NN", "10NN", "15NN", "LDA", "Logit", "SVM","ANN")))
# add for the AUC
pr.auc <-  matrix(-1, nrun, nmethod, dimnames = list(paste0("run", 1:nrun),c("1NN", "5NN", "10NN", "15NN", "LDA","Logit", "SVM", "ANN")))
for (i in 1:nfold) {
  ...
  ANN <- nnet(group ~ ., data = glm.dat,size=4)
  pr.ann <- predict(ANN, newdata = data.frame(t(testdat.fold)))
  pr.err[i, "ANN"] <- mean(pr.ann)
